...
if (!policy.includes("script-src 'self';") || !policy.includes("script-src-attr 'none'")) {
  throw new Error('expand.systems must allow only same-origin script files and no event attributes')
}

const failures = []
for (const file of fs.readdirSync(root).filter(name => name.endsWith('.html'))) {
  const html = fs.readFileSync(path.join(root, file), 'utf8')
  if (/<script(?![^>]*\bsrc=)[^>]*>/i.test(html)) failures.push(`${file}: inline script`)
